...
impl Hand {
    ...
    fn update_strength(&mut self) {
        let mut rank_counter: HashMap<&char, usize> = HashMap::new();
        ...
        let j_count = match rank_counter.get(&'J') {
            Some(j_count) => *j_count,
            None => 0,
        };

        if j_count > 0 && j_count < self.cards.len() {
            rank_counter.remove(&'J');

            let mut max_rank = &' ';
            let mut max_count = &0_usize;

            for (rank, count) in &rank_counter {
                if count > max_count {
                    max_rank = rank;
                    max_count = count;
                }
